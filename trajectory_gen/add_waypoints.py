...
def add_waypoints(input_file, output_file, waypoints_file):
    ...
    for idx, pos in enumerate(traj_df.values[0::int(space)], start=1):
        # create a waypoint subelement
        ET.SubElement(worldbody, 'geom', {'name': 'waypoint{}'.format(idx),
                                          'pos': '{} {} 0.0 '.format(pos[0], pos[1]),
                                          'size': '0.03 0.03 0.03',
                                          'type': 'sphere',
                                          'contype': '0',
                                          'rgba': color})
...
if __name__ == "__main__":
    add_waypoints("../model/plane_arm_big.xml", "../model/plane_arm_big_waypoints.xml", "./robot_traj_resize.csv")
